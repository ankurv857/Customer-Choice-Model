...
if __name__ == '__main__':
    ...
    departments = pd.read_csv(os.path.join(path, "departments.csv"), dtype={'department_id':np.uint8, 'department': np.unicode_})
    order_prior = pd.read_csv(os.path.join(path, "order_products__prior.csv"), dtype={'order_id': np.uint32,
                                                                                      'product_id': np.uint16,
                                                                                      'add_to_cart_order':np.uint8,
                                                                                      'reordered': bool})
    ...
    labels = pd.read_pickle('/Users/ankur/Documents/Competitions/Insta/previous_products.pkl')
    orders = orders.loc[(orders.eval_set == 'train') | (orders.eval_set == 'test'), :]
    labels = pd.merge(labels, orders[['order_id', 'user_id', 'eval_set']], on='user_id').drop(['user_id'], axis=1)
order_train.drop(['add_to_cart_order'], axis=1, inplace=True)
print('data is loaded')
...
for fold in range(folds):
    ...
    print(current.columns)
    print(current.shape)
current.to_pickle('/Users/ankur/Documents/Competitions/Insta/chunk_{}.pkl'.format(fold))
